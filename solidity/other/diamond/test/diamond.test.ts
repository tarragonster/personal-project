...
describe("DiamondTest", async () => {
  ...
  it("should have four facets -- call to facetAddresses function", async () => {
    ...
    totalInitAddresses = addresses.length;
  ...
  it("facets should have the right function selectors -- call to facetFunctionSelectors function", async () => {
    ...
    assert.sameMembers(result, selectors);
    selectors = getSelectors(operationFacet);
  ...
  it("should replace supportsInterface function", async () => {
    ...
    const testFacetAddress = addresses[totalInitAddresses];
    tx = await diamondCutFacet.diamondCut(
      [
        {
  ...
  it("should remove some test2 functions", async () => {
    ...
    result = await diamondLoupeFacet.facetFunctionSelectors(addresses[totalInitAddresses + 1]);
  ...
  it("should remove some test1 functions", async () => {
    ...
    result = await diamondLoupeFacet.facetFunctionSelectors(addresses[totalInitAddresses]);
  ...
  it("add most functions and facets", async () => {
    ...
    const cut = [
      ...
      },
      {
        ...
        action: FacetCutAction.Add,
        functionSelectors: getSelectors(operationFacet),
      },
      {
        facetAddress: addresses[totalInitAddresses],
        action: FacetCutAction.Add,
        functionSelectors: getSelectors(Test1Facet),
      },
      {
        facetAddress: addresses[totalInitAddresses + 1],
        action: FacetCutAction.Add,
    ...
    assert.equal(facetAddresses.length, totalInitAddresses + 2);
    assert.equal(facets.length, totalInitAddresses + 2);
    ...
    assert.sameMembers(facets[findAddressPositionInFacets(addresses[3], facets)][1], getSelectors(operationFacet));
    assert.sameMembers(facets[findAddressPositionInFacets(addresses[totalInitAddresses], facets)][1], getSelectors(Test1Facet));
    assert.sameMembers(facets[findAddressPositionInFacets(addresses[totalInitAddresses + 1], facets)][1], getSelectors(Test2Facet));
  });
  ...
  it("should work with Huuk Market functions", async () => {
    ...
    result = await diamondLoupeFacet.facetFunctionSelectors(huukMarketFacet.address);
    assert.sameMembers(result, selectors);

    const huukMarket = await ethers.getContractAt("HuukMarketFacet", diamondAddress);
    const facets = await diamondLoupeFacet.facets();
    assert.sameMembers(facets[findAddressPositionInFacets(addresses[addresses.length - 1], facets)][1], getSelectors(huukMarket));
  });
});
